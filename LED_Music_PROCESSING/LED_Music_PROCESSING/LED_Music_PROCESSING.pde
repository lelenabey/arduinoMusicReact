...
int ledPin =  10;    // LED connected to digital pin 12
int ledPin2 =  11;    // LED connected to digital pin 1
  ...
  for(int i = 0; i < 120; i++)
  {
    if (i<=1)
    {
      if (vectormedias[0]<2*fftLin.getAvg(i))
        ...
        arduino.analogWrite(ledPin, int(vectormedias[0]));
    ...
    {
      if (vectormedias[2]<2*fftLin.getAvg(i))
        vectormedias[2]=2*fftLin.getAvg(i);
        arduino.analogWrite(ledPin, int(vectormedias[2]));
    }
    if (i<=160 && i>=70)
    {
